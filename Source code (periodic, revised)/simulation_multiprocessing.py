...
#     stop_folder = os.path.join(base_dir, f"March25_Case_{case}_StopToStop_{stop_to_stop_distance}")
...
#         10000, 4000, density, kappa, 20, safe_stopping_speed,
...
#     num_cores = min(mp.cpu_count(), 29)  # Use up to 24 cores
...
def run_simulation(trial_info):
    ...
    # Base directory for saving results
    base_dir = os.path.abspath(os.path.join(os.getcwd(), ".."))
    stop_folder = os.path.join(base_dir, f"April1_Case_{case}_StopToStop_{stop_to_stop_distance}")
    arrival_folder = os.path.join(stop_folder, f"Kappa_{kappa}", f"Density_{density}", f"PassengerRate_{arrival_rate}")

    # Create required subfolders
    subfolder_paths = {name: os.path.join(arrival_folder, name) for name in
                       ["VehicleData", "TimestepSummary", "PassengerData", "SpatioTemporal"]}
    for folder in subfolder_paths.values():
        os.makedirs(folder, exist_ok=True)
