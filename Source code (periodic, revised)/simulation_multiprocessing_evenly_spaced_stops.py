...
def run_simulation(trial_info):
    ...
    sidewalk = Sidewalk(length=4860, width=1, max_passengers_per_cell=20)
    road_designation = Road(length=4860, width=4, speed_limit=5, allowed_rows=params.allowed_rows_input)
    ...
    stop_folder = os.path.join(base_dir, f"April1_Case_{case}_StopToStop_{stop_to_stop_distance}_Evenly_Spaced_Stops")
    ...
    timestep_summary, passenger_data, vehicle_data, spatio_temporal, sidewalk_patio_temporal = integrated_simulator.run_simulation(
        150000, 75000, density, kappa, stop_to_stop_distance, safe_stopping_speed,
...
def run_simulations_for_params(params_file):
    ...
    densities = [0.8]
    passenger_arrival_rates = [1]#0.3 AND 1
    stop_to_stop_distances = [162] #162, 378, 648
    kappa_values = [0.2]
    num_trials = 1
