...
class Passenger_Simulator:
    ...
    def update_occupancy(self):
        ...
        for x_position in range(self.sidewalk.length):
            number_of_passengers_present = len(self.sidewalk.passengers[x_position])
            ...
            #print(f"Hence, sidewalk occupancy at position {x_position} is {self.sidewalk.sidewalk_occupancy[x_position]}.")
    ...
    def generate_passengers(self, vehicle_simulator, current_time_pass):#Put an algorithm that generates passengers only on designated stops
        ...
        for position in range(0, self.sidewalk.length):
            ...
            if len(self.sidewalk.boarding_stops[position]) != 0: #We spawn passengers on sidewalk cells with designated stops only (Here, we check for the presence of a stop)
                if np.random.random() < self.passenger_arrival_rate:
                    if self.sidewalk.occupancy[position, 0] < self.sidewalk.max_passengers_per_cell:
                        ...
                        destination = None 
                        set_of_alighting_stops = self.sidewalk.alighting_stops
                        
                        for stop_list in set_of_alighting_stops:
                            for stop in stop_list:
                                if stop.position  == position:
                                    destination = stop #Dirac-delta distribution of distances

                        #print(f"The destination position for the passenger initiated at position {position} is {destination_position}")
