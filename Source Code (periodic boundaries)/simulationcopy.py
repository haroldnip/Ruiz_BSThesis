...
#-----------------------------TEST RUN------------------------------------------------------------
#integrated_simulator.run_simulation(500, 1, 1, 10, 0.2, 0, 1, safe_stopping_speed, safe_deceleration, jeepney_allowed_rows, truck_allowed_rows, visualize=True)
...
# folders = ["VehicleData196", "TimestepSummary196", "PassengerData196"]
...
#     timestep_summary, passenger_data, vehicle_data = integrated_simulator.run_simulation(1000, 1, 1, 200, density, truck_fraction, 1,safe_stopping_speed, safe_deceleration, jeepney_allowed_rows, truck_allowed_rows, visualize=False)

#     # File paths (outside notebook
#     # File paths (outside the notebook folder)
#     vehicle_file = os.path.join(folder_paths["VehicleData196"], f"Trial_{trial}(density = {density}).csv")
#     timestep_file = os.path.join(folder_paths["TimestepSummary196"], f"Trial_{trial}(density = {density}).csv")
#     passenger_file = os.path.join(folder_paths["PassengerData196"], f"Trial_{trial}(density = {density}).csv")
...
for trial in range(1, num_trials + 1):
    ...
    timestep_summary, passenger_data, vehicle_data, spatio_temporal = integrated_simulator.run_simulation(6000, 1, 1, 1000, density, truck_fraction, 1,safe_stopping_speed, safe_deceleration, jeepney_allowed_rows, truck_allowed_rows, visualize=False)

    # File paths (outside notebook
    # File paths (outside the notebook folder)
    vehicle_file = os.path.join(folder_paths["VehicleData_test"], f"Trial_{trial}Mar6(density = {density}, truck_fraction = {truck_fraction}).csv")
    timestep_file = os.path.join(folder_paths["TimestepSummary_test"], f"Trial_{trial}Mar6(density = {density}), truck_fraction = {truck_fraction}.csv")
    passenger_file = os.path.join(folder_paths["PassengerData_test"], f"Trial_{trial}Mar6(density = {density}), truck_fraction = {truck_fraction}.csv")
    spatio_temporal_file = os.path.join(folder_paths["Spatio-Temporal_test"], f"Trial{trial}Mar6(density = {density}), truck_fraction = {truck_fraction}.csv")
    # Save CSV files
    vehicle_data.to_csv(vehicle_file, index=False)
    timestep_summary.to_csv(timestep_file, index=False)
    passenger_data.to_csv(passenger_file, index=False)
