...
class Vehicle:
    ...
    def collect_passenger_destinations(self):
        ...
        if self.vehicle_type == "jeep":
            ...
            for passenger in self.passengers_within_vehicle:
                ...
                if passenger.riding_status == "let me out" and (passenger.edge_crossings == 1 or passenger.about_to_cross_edge): #This assumes a Dirac Delta function (where a = road length)
                    #print(f"One passenger in {self.vehicle_type} {self.vehicle_id} says manong para po.")
                    #if passenger.destination_adjacent_to_vehicle or passenger.overshoot_destination:
                    passenger_destinations.add(passenger.destination_stop)
    ...
    def determine_if_overshot_destination(self):
        ...
        self.overshot_destination = any(passenger.overshoot_destination for passenger in self.passengers_within_vehicle)
    ...
    def calculate_nearest_destination_distance(self): #This is not final, I stopped right here (Feb. 13, 2025, 9:26 PM)
        ...
        # if nearest_distance.size > 0 and (nearest_distance < 0).any(): 
        # # print(f"Error, negative destination distance: {self.vehicle_type} {self.vehicle_id}, {nearest_distance[0]}")
        #     pass
        return max(0, nearest_distance[0] - 1) if nearest_distance.size > 0 else inf #Try minus 1 instead of minus 2
    ...
    def matching_stops_on_sidewalk(self, passenger_destinations):
        """Check if any occupied position has a matching alighting stop in passenger destinations."""
        self.update_pool_of_occupied_positions()
        ...
        for position in self.pool_of_occupied_positions:
            if any(stop in passenger_destinations for stop in self.sidewalk.alighting_stops[position]):
                self.matching_stop_found = True
                # print(f"{self.vehicle_type} {self.vehicle_id} in {self.pool_of_occupied_positions} detected matching stops on sidewalk.")
    ...
    def unload_passengers(self, unloaded_passengers):
    ...
    def unloading(self, unloaded_passengers): #Deceleration and lane changing of vehicle was already handled
        ...
        if self.vehicle_type == "jeep":
            ...
            if self.will_unload_passengers() and self.speed == 0:
                ...
                # print(f"{self.vehicle_type} {self.vehicle_id} has matching stop found??? --- {self.matching_stop_found}.")
                # print(f"{self.vehicle_type} {self.vehicle_id} overshot passenger destination???? ---- {self.overshot_destination}.")
                if self.matching_stop_found or self.overshot_destination:
                    ...
                    if self.current_row == 0: #The deceleration method stops the vehicle to v = 0 if needed
                        self.update_pool_of_occupied_positions()
                        self.unload_passengers(unloaded_passengers)               
