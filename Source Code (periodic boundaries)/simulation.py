...
for trial in range(1, num_trials + 1):
    ...
    # File paths (outside notebook
    # File paths (outside the notebook folder)
    vehicle_file = os.path.join(folder_paths["VehicleData_test"], f"Trial_{trial}Mar5(density = {density}, truck_fraction = {truck_fraction}).csv")
    timestep_file = os.path.join(folder_paths["TimestepSummary_test"], f"Trial_{trial}Mar5(density = {density}), truck_fraction = {truck_fraction}.csv")
    passenger_file = os.path.join(folder_paths["PassengerData_test"], f"Trial_{trial}Mar5(density = {density}), truck_fraction = {truck_fraction}.csv")
    spatio_temporal_file = os.path.join(folder_paths["Spatio-Temporal_test"], f"Trial{trial}Mar5(density = {density}), truck_fraction = {truck_fraction}.csv")
    # Save CSV files
    vehicle_data.to_csv(vehicle_file, index=False)
    timestep_summary.to_csv(timestep_file, index=False)
    passenger_data.to_csv(passenger_file, index=False)
